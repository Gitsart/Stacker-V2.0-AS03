...
void send_data(String data_display) {
  ...
  if (data_display == "1") {
    ...
    rampo=200;
    fwdSpeed = 150;
    bwdSpeed = 1800;  // Higher speed for revStroke
    ...
    rampo = 150;
    fwdSpeed= 450;
    bwdSpeed = 1800;  // Higher speed for anticlockwise
    motorShouldRun = false; // Reset motor run flag
...
void motor() {
  ...
  if (motorShouldRun && !clothDetected)
  {
    ...
    while (stepper1.moving())
    {
      stepperPause.setTime(100); // Wait until the backward movement is complete
    }
